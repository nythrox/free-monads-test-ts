...
function runGenerator(gen, arg, then) {
  ...
  function* handle(fn, handlers) {
    return yield (lastGen, lastThen) => {
      ...
      thisThen.i = num;
      num++;
      const subGen = typeof fn === "function" ? fn() : fn;
  ...
  function* perform(name, values) {
    console.log(`perform: (${name}, ${values})`);
      ...
      const handlerGen = handler(values, function* (val) {
        ...
        return yield (__, __undefined) => {
          ...
          resume(currGen, val, (handlerVal) => {
            resume(handlerGen, handlerVal, _then); // TODO: why does chaning _then for then change the result in a weird way?
          });
        };
      });
      start(handlerGen); //start handlerGen, it gets interrupted and someone else handles it
    };
  }
  ...
  function* test2() {
    ...
    const plusOne = yield* perform("plusOne", 1);
