...
const [ask, isAsk] = toFree<Console, 'ask'>();
const [write, isWrite] = toFree<Console, 'write'>();
...
const [set, isSet] = toFree<LocalStorage, 'set'>();
...
describe('free', () => {
  ...
  test.only('async free', async () => {
    ...
    const result = await run(program)(runProgram2);
...
const test3 = doFree(function* () {
  const [store, result] = yield* state(10, counter());
  return store;
});
...
const sla = algumacoisa<number>('100').handle(handleExn((err) => 10));

type ExnVal<T> = T extends Exn<infer A, infer B> ? A : never;
type ExpectedErrRet<T> = T extends Exn<infer A, infer B> ? B : never;
